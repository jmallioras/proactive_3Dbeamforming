...
def plot_continuous_distribution(data, column_index):
    ...
    # Plot lines for mean and mean +/- std deviations
    colors = ['red', 'green', 'purple', 'orange']
    for i, num_std in enumerate([0, 1, 2, 3]):
        plt.axvline(mean_val + num_std * std_val, color=colors[i], linestyle='dashed', linewidth=1,
                    label=f'Mean + {num_std} STD' if num_std > 0 else 'Mean')
